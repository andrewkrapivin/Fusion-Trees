...
int pc_init(pc_t *pc, int64_t *global_counter, uint32_t num_counters, int32_t threshold) {
  ...
  pc->local_counters = (lctr_t *)calloc(pc->num_counters, sizeof(*pc->local_counters)); // Not guaranteed to be aligned in a cache line??
  // pc->local_counters = (lctr_t *)aligned_alloc(64, 64*pc->num_counters); //except this seems marginally slower lol, so idk
  // memset(pc->local_counters, 0, 64*pc->num_counters);
